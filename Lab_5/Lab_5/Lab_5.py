...
if __name__ == '__main__':
    ...
    for i in range(0, n):
        ...
        pointRT = Funcs.GetRtPoint()
        temp = Funcs.PTriangle(pointRT[0], pointRT[1], pointRT[2])
        pArr.append(temp)

        #генеруємо вершини рівнобедренного трикутника
        pointIT = Funcs.GetItPoint()
        temp = Funcs.ITriangle(pointIT[0], pointIT[1], pointIT[2])
        iArr.append(temp)

        # генеруємо вершини рівностороннього трикутника
        pointET = Funcs.GetEtPoint()
        temp = Funcs.ETriangle(pointET[0], pointET[1], pointET[2])
        eArr.append(temp)
